...
var defaultChecks = [
  ...
  },
  {
    ...
    errorText: '这是无效的电话。',
    check: function (self, object) {
      return (/^[0-9\-]{0,20}$/i).test($(object).val()) ? true : self.errorText
    }
  },
  {
    type: 'url',
  ...
  },
  {
    ...
    errorText: '无效的银行账号。',
    check: function (self, object) {
      return (/^([0-9]{12,35})?$/g).test($(object).val()) ? true : self.errorText
    }
  },
  {
    ...
    errorText: '必输项。',
    check: function (self, object) {
      return $(object).data('size') ? true : self.errorText
    }
  },
  {
    type: 'password',
  ...
  },
  {
    ...
    errorText: '无效的传真号。',
    check: function (self, object) {
      return (/^(\d{3}-\d{8})|(\d{4}-\d{7,8})$/).test($(object).val()) ? true : self.errorText
    }
  },
  {
    type: 'floatMoney',
    ...
    check: function (self, object) {
      ...
      const val = $elm.val()
      // 自动修正数据的展现
      if (/(^[1-9][0-9]*(.[0-9]{1,2})?$)|(^[0-9].[0-9]{1,2}$)/.test(val)) {
        $elm.val((val - 0).toFixed(2))
        return true
      }
...
class formChecker {
  constructor (config) {
    ...
    console.log('formChecker init:', this)
    ...
    $targets.bind('change', function () {
      ...
      $(this).removeClass('checker-color-invalid')/* from sx-9067 sx-wangc@dtdream.com */
  ...
  tips (obj, strings, show) { /* 错误提示接口 */
    if ($(obj).attr('checker-text') !== 'false') {
      ...
      if (!($(obj).is('select') || $(obj).parent().hasClass('selectric-hide-select'))) {
        ...
        $append = ($append.length === 0) ? $(obj) : $append
  ...
  colorBorder (obj, status) {
    var $obj = $(obj)
    /* select特殊处理 */
    if (($(obj).is('select') === true) && ($(obj).parent().hasClass('selectric-hide-select') === true)) {
  ...
  patternCheck (obj) {
    ...
    if (undefined === checker) {
      ...
      if (undefined === errorText) { errorText = '输入错误！' }
  ...
  isRequired (obj) {
    ...
    } else if ($(obj).is('select') === true) {
      return (($(obj).val() === null) || ($(obj).val().trim().length === 0)) ? false : true
    } else {
      return ($(obj).val().trim().length === 0) ? false : true
    }
  }
  ...
  lengthCheck (obj) {
    ...
    console.log($(obj), maxLen, length, minLen)
    $(obj).siblings('span[name="check-lengtherr"]').remove()
    ...
    if ((undefined !== maxLen) && (length > maxLen)) {
      ...
      that.tips(obj, '<span name="check-lengtherr" class="text-danger pl">输入不能超过' + maxLen + '位</span>')
    }
    if ((undefined !== minLen) && (length < minLen)) {
      /* 过短 */
      $(obj).addClass('length-invalid').removeClass('valid')
      that.tips(obj, '<span name="check-lengtherr" class="text-danger pl">输入不能短于' + minLen + '位</span>')
    }
  }
